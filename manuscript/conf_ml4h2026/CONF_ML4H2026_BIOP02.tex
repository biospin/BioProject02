...
\subsection*{R5. Foundation Models}
...
\subsection*{M5. Cost-of-Substitution Frame}
...
\section*{Appendix A. R5 Foundation-Model Robustness}
...
The main text carries exactly one figure: Figure 1, the observed map overlaying misassignment loss on the confusion matrix weighted by therapeutic distance (\texttt{figures/fig01\_cost\_map.pdf}). This figure fixes both ends of the spectrum in a single panel.

The remaining figures are placed in the appendix, where the CFP imposes no length limit.

\textbf{Figure A1} Per-axis misassignment loss and the confidence interval of the headline contrast (\texttt{figures/figA1\_axis\_cost.pdf}).

\textbf{Figure A2} Power ceiling, holdout positives per axis and the boundary of decidability (R2, Table R2) (\texttt{figures/figA2\_power\_ceiling.pdf}).

\textbf{Figure A3} Site confounding audit, site-label association per endpoint (R1 footnote, Appendix C M9) (\texttt{figures/figA3\_site\_confounding.pdf}).

\textbf{Figure A4} Multi-model comparison, order preservation across UNI, Virchow2 and UNI2-h and the diverging single endpoints (Appendix A, Table R5) (\texttt{<FILL: not yet rendered>}).
