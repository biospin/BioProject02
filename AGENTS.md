...
> 버전: v0.7 | 작성: 2026-05-16 braveji (ykji) | 최종 수정: 2026-06-30 (SSH 포트 통일 — CLAUDE.local.md/COLLAB 기준)
...
> **서버 안내(현행):** 작업 서버는 `121.126.38.195`(RTX A6000 49GB × 3, 내부망 `192.168.0.85`). 접속은 **직접접속** `ssh -p <포트> <계정>@121.126.38.195`(2026-07-11 정본). 구 bastion(`61.109.239.220`, A100 서버 시절)은 **폐기** — 더 이상 쓰지 않는다. 위 SSH 포트는 2026-06-30 현행 서버에 맞춰 정정됨(표 기준).
