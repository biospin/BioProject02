...
"진행 사항 알려줘 / 중간 과정 어떻게 돼? / 앞으로 뭐 할까 / 더 할 게 있나" 요청을 처리한다. **기억이 아니라 실측**으로 답한다(다른 창이 동시에 작업하므로 내 세션 기억은 최신이 아니다 — `docs/PITFALLS_REGISTRY.md` C2·C5).

## 🔴 0순위 — 무엇보다 먼저 세션 기록을 읽는다 (C5 재발방지)
"진행사항/할 일/앞으로 뭐/더 할 게 있나" 물음의 **1순위 소스는 git·JIRA가 아니라 세션 기록**이다. git·JIRA만 보면 이미 끝났거나 이미 결정된 것을 "할 일"로 오보한다(C5 실사고).
1. `HANDOFF.md` **최상단 30줄** — 최근 세션이 무엇을 완료/결정했는지.
2. `SESSION_LOG.md` **최근 1~2일 항목**.
3. **그 다음** git fetch·JIRA로 교차검증(사람 회신·팀 커밋).
"이거 아직 안 됐나?"는 반드시 세션 기록에서 먼저 확인하고 나서 제안한다.
