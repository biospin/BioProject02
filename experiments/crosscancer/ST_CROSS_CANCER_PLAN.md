...
- **높은 가치의 단 하나 = 대장 MSI-가시 / RAS-조용 공간 대조**(Angle A의 대장 격):
  - RAS/EGFR-경로 발현이 공간적으로 존재하나 형태학적으로 조용 → ERBB2 floor 대칭, 항EGFR 축 고비용의 메커니즘 근거.
...
2. **대장 ST 그림** — 위 조사가 Go일 때만. Angle A식 threshold-free 지표로 (a)RAS/EGFR 형태-조용 (b)MSI-면역 형태-공존 대조. 전면 ST 트랙(D9 위반) 아님 — 타깃 방어 그림 1건.
...
- **판정: 대장 ST = adjacent(인접·미스쿱, 스쿱 위험 낮음) · 데이터 conditional-Go.** 정확히 우리 프레이밍(MSI-면역 형태공존 vs RAS/EGFR 형태-조용 decoupling을 치환비용으로 시연)을 한 선행 없음. 최근접 5건(Path2Space 유방전용·반대방향, Valdeolivas CMS이질성 기술, Yang KRAS niche 기술, Pelka MMRd 면역hub 주로 scRNA, arXiv MSI예측)은 전부 결이 다름.
- **⚠️ 중대 caveat(유방과 구조적 차이):** ERBB2는 증폭→과발현이라 ST가 치료축을 직접 읽지만(Angle A가 깨끗한 이유), **KRAS/NRAS/BRAF는 점변이라 발현량이 안 변한다** → "RAS-조용"은 하류 프록시(MAPK/RAS 전사 시그니처·EGFR 리간드 EREG/AREG) 경유. 본질적으로 유방 ERBB2 floor보다 덜 깨끗. 숨길 결함이 아니라 "대장이 유방보다 어렵다"는 정직한 포지셔닝. **아형↔변이가 아니라 변이↔변이지만, 증폭↔점변이라 ST 가독성 층위가 다름 — 그림에서 대칭인 척 금지.**
- **데이터 병목:** MSI-H + MSS(+RAS)를 라벨과 함께 H&E-정합 Visium으로 담은 단일 공개셋 부재.
  - RAS-조용 반쪽 = 즉시 가능: **Valdeolivas(Zenodo 7551712, 7pt/14 Visium+H&E+annot)=1순위**, **Yang(GSA HRA011642, 2 KRAS-mut+2 WT)=RAS 페어 보강**.
