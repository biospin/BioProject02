...
3. **위암 양성대조(Lauren) FAIL(0.54) = Lauren 국한 site-교란(원인 진단 완료, `GASTRIC_STAD/full/LAUREN_POSCONTROL_DIAGNOSIS.md`):** 같은 파이프라인의 위 MSI는 dev 0.899→holdout 0.860으로 정상 일반화하는 반면 Lauren만 0.963→0.536으로 무너졌고(pixel-mean 0.631 > MIL 0.536), 기관별 유병률 100%~18% + 기관분리 분할이 고유병 기관을 평가셋(88%)에 몰아넣은 것이 직접 원인이다. → **저신뢰는 Lauren에 국한하고, 같은 코호트의 MSI 등 다른 축은 유효하게 남는다.** (원고 R4와 정합 — 이전 "위암 endpoint 전체 저신뢰"는 원인 진단 전 표현으로, 진단 완료로 Lauren 국한으로 정정. 정정일 2026-07-27, BIOP02-105.)
