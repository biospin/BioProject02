...
| A2 | kkkim | Yale 코호트(TCIA HER2-TUMOR-ROIS) 확보·타일·UNI 임베딩 | ✅ **완료 2026-07-17** — **276/276**(trastuzumab 85 + HER2 191), failed=0. PathDB JSON API로 headless 다운로드. 검증(실물): 전부 1024-d·손상0·finite, 타일 median 2124(cap 5000). 출력 `/workspace/data/cache/biop02/yale/uni_v1/`, manifest `embedding_manifest_yale_uni.csv`. 파이프라인 PR #59 |
| A3 | sjpark | frozen phenotype 모델을 Yale 임베딩에 적용(항HER2 축 점수 산출) | **착수 가능**(재료 완비, 통지 BIOP02-80 c.11283·11284) |
| A4 | jhans | **[전환]** Yale 앵커 소유 — 항HER2 축 점수 → pCR 층화(AUROC+bootstrap CI, DeLong vs HER2-prob baseline), 반증조건 보고 | **착수 가능**. ★검정 정밀화(Farahmand 심층분석): 성공=frozen-transfer가 **CV AUC 0.80[CI 0.69–0.88]에 근접/겹침**(이긴다 아님 — 그들=in-cohort CV천장), 진짜검정=**DeLong vs HER2-확률 baseline**(전부 HER2+ 코호트) |
