...
- **가용성(2026-07-12 확인)**: 로컬=UNI·CONCH·EXAONE만. **Virchow2/UNI2/GigaPath=게이팅+미다운로드**. EXAONE=coords 비호환(블로커). CONCH=로컬이나 UNI보다 약해 역량-견고성 논증엔 부족(아키텍처 다양성 증거로만).
- **[사용자 액션] HF 게이팅 신청(기관 이메일, @gmail 불가)**: `paige-ai/Virchow2`(1순위), `MahmoodLab/UNI2-h`(2순위), (선택)`prov-gigapath/prov-gigapath`.
