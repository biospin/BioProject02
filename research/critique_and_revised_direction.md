...
## 수정된 척추 (리뷰어 제안 — ⚠️ Leader가 일부 번복, 아래 "Leader 결정 확정" 참조)

> 이 섹션은 **3개 에이전트의 권고안 원본 기록**이다. Leader(kkkim)는 결정 #1에서 "치료층 강등"을 **불채택**하고 치료층을 보강해 유지하기로 했다. 즉 아래 "엄밀성/auditability를 헤드라인으로"는 채택하되, "치료=경계모듈로 강등"은 **치료=보강된 헤드라인 기여**로 대체된다.
...
## Leader 결정 확정 (2026-06-10, Leader=kkkim)

> 역할: **Project Leader = kkkim** (gglee 이탈, 이양). Critic 총괄 = braveji. (CLAUDE.md/AGENTS.md 정식 갱신은 `docs/BIOP02-kkkim-roles-update` 브랜치 → main 머지 대기 중.)

1. **헤드라인: 치료층 유지 (강등 거부).** 리뷰어 권고(치료=경계모듈로 강등)를 **불채택**. Leader가 치료·신약개발을 핵심 관심축으로 두므로 치료-가설 층을 **헤드라인 기여로 유지**하되, "trivial·검증불가" 평결을 정면 반박할 **보강(데이터·방법·아이디어)을 추가**하여 *falsifiable한 치료층*으로 격상한다. → 보강안 = [`therapeutic_layer_strengthening.md`](therapeutic_layer_strengthening.md) (별도, research-methodologist 설계).
   - 단, 구멍 #1~#3은 그대로 유효 → 보강안이 반드시 (a) 실제 결과 앵커, (b) 비-collinear 독립 readout + shuffle-null, (c) Critic 비순환을 충족해야 헤드라인 자격 인정. "관심 있으니 유지"가 rigor 면제는 아님.
2. **Exp3 = Paper A 포함 (보류 해제).** reviewer 단일 최중요 권고 수용. benchmark→method 격상의 유일 수단(auditability 실증). experiment_plan.md "Exp3 보류" 결정 역전. GPU 최대 항목이나 Paper A 척추로 확정.
3. **Exp7(발현 시그니처) = 포함, 전이 연료 한정.** categorical 병목은 그대로 유지(auditability 사수) → 구멍 #1(richness↔auditability 모순) 회피. 시그니처는 치료층 route2(signature-reversal)·CRISPR/LINCS 수렴의 **연료로만** 사용, 병목으로 승격 금지. ssGSEA/GSVA는 **train-only fit**(누수 차단).

**해소된 자기모순 정리.** 구멍 #1(richness↔auditability)은 "병목=categorical 유지, 시그니처=별도 전이연료" 분리로 해소됨 → Exp7 포함과 Exp3 auditability가 양립.
