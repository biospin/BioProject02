...
`ai_scientist/`의 설계서 7개(README + 01~06)를 **mermaid 13종 + HTML** 단일 페이지로 시각화한 결과물.
...
| **3** | **⭐ 메타-학습 루프** — 사고→PITFALLS_REGISTRY→**재발방지 장치** | 신규 (kkkim ④) |
| 4 | 2-레이어 + 사람 게이트 3개 | 01 |
| **5** | **⭐ 레이어 A 상세** — 5역할 체인 + **A→B 산출물 계약** | 신규 (kkkim ②) |
| 6 | **권한 강제 지도** — 도구로 막힌 곳 vs 말로만 | 02 (GAP 3) |
| 7 | mock 분기 — 데모를 실 결과처럼 넘기지 않는 장치 | 03 |
| 8 | 검수 3층 — CI 7종 상세 | 04 |
| 9 | **검증기를 검증하는 층**(mutation / 공허통과) | 04 |
| **10** | **⭐ 세션을 넘는 상태 인계** — HANDOFF/SESSION_LOG 2층 + memory | 신규 (kkkim ①) |
| 11 | 협업 흐름 | 05 |
| 12 | 교차검수 owner≠reviewer | 05 |
| 13 | 설계 계보 | 06 |

## kkkim(Leader) 검토 반영 — #11983 (2026-08-20)

| 지적 | 반영 |
|---|---|
| ① **세션-간 상태 지속 레이어가 없다**(가장 큰 누락) | ✅ 신설 섹션 + 그림 10 — HANDOFF/SESSION_LOG 2층 · memory · 백업 규칙 · 로그 유실 사고 |
| ② **Layer A가 얕다** | ✅ 그림 5 신설 — 5역할 체인(data→embedding→modeling→therapeutic_evidence→critic) + **A→B 산출물 계약** |
| ③ 헤드라인 재계산 게이트가 CI에 없음 | ✅ GAP 1에 명시 — *"CI의 숫자 드리프트는 JSON↔md 정합이지 raw 재산출이 아님"*. **CI화는 Leader 승인 사안으로 남김** |
| ④ incident→레지스트리→가드 루프에 이름 붙이기 | ✅ 그림 3 신설 — **메타-학습 루프** |

> ③의 경계는 kkkim 지적대로다 — **문서에 갭으로 적는 것까지는 지금, 실제 CI 신설은 별도 승인.**
> `/wrap`·`/mine`은 이 환경에서 실재를 확인하지 못해 **인용하지 않았다.**
...
| `auto_review_config.json` | `ai_review.agents` → **`["paper-critic"]` 정렬 완료**(kkkim 승인 #11915) → 갭 아님(닫힘) |
