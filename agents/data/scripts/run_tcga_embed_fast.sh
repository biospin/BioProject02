#!/usr/bin/env bash
# TCGA-BRCA UNI 임베딩 (FAST) — SSD 스테이징 + 병렬 타일읽기. 기존 타일 활용, resumable.
...
EXTRACTOR="agents/embedding/scripts/extract_uni_fast.py"
K=3
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, extractor, "--coords", coords_path, "--out_dir", emb_dir,
             "--batch_size", "64", "--device", gpu, "--num_workers", "6"],
            check=False
        )
