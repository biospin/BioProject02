"""
CONCH tile embedding extractor (MahmoodLab/CONCH, 512-dim).

Reads tile coordinates from coords.npy + coords.json (produced by tile_wsi.py),
extracts per-tile embeddings using CONCH, and saves as float32 .npy.

Requirements:
    pip install conch   # or: pip install git+https://github.com/mahmoodlab/CONCH.git
    export HF_TOKEN=hf_xxx   # gated model — HuggingFace access token 필요

Run:
    export LD_LIBRARY_PATH=~/miniconda3/lib:${LD_LIBRARY_PATH:-}
    time python agents/embedding/scripts/extract_conch.py \
        --coords /workspace/data/cache/biop02/tiles/TCGA-xxx_coords.npy \
        --out_dir /workspace/data/cache/biop02/embeddings/conch_v1/ \
...
def load_conch(device: str) -> tuple:
    """Load CONCH from HuggingFace hub. Returns (model, preprocess)."""
    from conch.open_clip_custom import create_model_from_pretrained

    print("  Loading CONCH from HuggingFace (MahmoodLab/conch) …")
...
def extract(
    ...
    emb_file = out_path / f"{slide_name}_conch_embeddings.npy"

    embeddings = np.empty((n_tiles, 512), dtype=np.float32)
    ...
    with torch.inference_mode():
        for start in tqdm(range(0, n_tiles, batch_size), desc="  Extracting", unit="batch"):
            ...
            # proj_contrast=False, normalize=False → raw 512-dim patch features
            feats = model.encode_image(tensor, proj_contrast=False, normalize=False)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract CONCH tile embeddings from a WSI"
    )
