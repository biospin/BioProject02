...
| ~~gglee~~ | [Geongyu](https://github.com/Geongyu) | (이탈 2026-06-09 — 리더→kkkim, Critic→braveji 재배정) | 2202 |
...
- **서버:** A100 80GB × 1, CPU 24코어, RAM 188 GiB, 저장소 2TB — `61.109.239.220`, SSH 키 전용
- **데이터 레이아웃:** 원본 WSI = S3 읽기 전용 | `/data/cache/` = LRU 200 GB | 임베딩 = 영구 보존
- **GPU 슬롯:** 09–13 / 13–17 / 17–21 / 21–01 (Slack 예약)
- **작업 공간:** `/workspace/agents/<role>/` (팀원별)

```bash
ssh -p <포트> <사용자명>@61.109.239.220
```
